...
<script>
    ...
    async function loadTrendData() {
        ...
        try {
            ...
            if (data.success && data.trend_data && data.trend_data.length > 0) {
    ...
    function renderTrendChart(trendData) {
        ...
        trendData.forEach((item, index) => {
            ...
            const formattedValue = item.count.toLocaleString();
            ...
            barsHtml += `
                <div class="group relative flex flex-col items-center justify-end h-full flex-1 min-w-[60px]">
                    <div class="relative w-full max-w-[60px] mx-auto">
                        <div class="w-full bg-gradient-to-t ${colors[colorIndex]} rounded-t-lg transition-all duration-300 hover:brightness-110 hover:scale-105 cursor-pointer"
                    ...
                    <span class="mt-2 text-xs font-medium text-gray-600 truncate w-full text-center px-1" title="${item.label}">
                        ${item.label.length > 10 ? item.label.substring(0, 8) + '...' : item.label}
                    </span>
                    <span class="text-[10px] font-bold text-[#155386]">${item.count}</span>
                </div>
            `;
        });
    ...
    function updateDonutChart(counts) {
        const total = (counts.pending || 0) + (counts.under_review || 0) + (counts.approved || 0) + 
                     (counts.rejected || 0) + (counts.for_release || 0) + (counts.verified || 0);
        ...
        donutChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [counts.pending || 0, counts.under_review || 0, counts.approved || 0, 
                           counts.for_release || 0, counts.verified || 0, counts.rejected || 0],
        ...
        legend.innerHTML = `
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2"><div class="w-3 h-3 bg-amber-500 rounded-full"></div><span class="text-sm text-gray-600">Pending</span></div>
                    <span class="text-sm font-bold text-gray-700">${pendingPercent}% (${counts.pending || 0})</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="bg-amber-500 h-1.5 rounded-full" style="width: ${pendingPercent}%"></div></div>
            </div>
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2"><div class="w-3 h-3 bg-blue-500 rounded-full"></div><span class="text-sm text-gray-600">Under Review</span></div>
                    <span class="text-sm font-bold text-gray-700">${underReviewPercent}% (${counts.under_review || 0})</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="bg-blue-500 h-1.5 rounded-full" style="width: ${underReviewPercent}%"></div></div>
            </div>
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2"><div class="w-3 h-3 bg-green-500 rounded-full"></div><span class="text-sm text-gray-600">Approved</span></div>
                    <span class="text-sm font-bold text-gray-700">${approvedPercent}% (${counts.approved || 0})</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="bg-green-500 h-1.5 rounded-full" style="width: ${approvedPercent}%"></div></div>
            </div>
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2"><div class="w-3 h-3 bg-purple-500 rounded-full"></div><span class="text-sm text-gray-600">For Release</span></div>
                    <span class="text-sm font-bold text-gray-700">${forReleasePercent}% (${counts.for_release || 0})</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="bg-purple-500 h-1.5 rounded-full" style="width: ${forReleasePercent}%"></div></div>
            </div>
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2"><div class="w-3 h-3 bg-emerald-500 rounded-full"></div><span class="text-sm text-gray-600">Completed</span></div>
                    <span class="text-sm font-bold text-gray-700">${verifiedPercent}% (${counts.verified || 0})</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="bg-emerald-500 h-1.5 rounded-full" style="width: ${verifiedPercent}%"></div></div>
            </div>
            ${counts.rejected > 0 ? `
            <div>
                <div class="flex items-center justify-between mb-1">
                    <div class="flex items-center gap-2"><div class="w-3 h-3 bg-red-500 rounded-full"></div><span class="text-sm text-gray-600">Rejected</span></div>
                    <span class="text-sm font-bold text-gray-700">${rejectedPercent}% (${counts.rejected || 0})</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="bg-red-500 h-1.5 rounded-full" style="width: ${rejectedPercent}%"></div></div>
            </div>
            ` : ''}
        `;
    }
        ...
        try {
            ...
            if (data.success && data.applications && data.applications.length > 0) {
                ...
                const statusText = {
                    ...
                    'rejected': 'Rejected', 'for-release': 'For Release', 'verified': 'Completed'
                };
                const gradients = ['from-[#155386] to-[#40798C]', 'from-[#40798C] to-[#70A9A1]', 'from-[#70A9A1] to-[#9EC5CB]', 'from-[#9EC5CB] to-[#B8D8E3]'];
                
                data.applications.slice(0, 5).forEach((app, idx) => {
                    ...
                    html += `
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-3 px-4"><div class="flex items-center gap-2"><div class="w-6 h-6 bg-gradient-to-r ${gradient} rounded-full"></div><span class="text-sm">${app.applicant_name || 'Unknown'}</span></div></td>
                            <td class="py-3 px-4 text-sm">${timeAgo}</td>
                            <td class="py-3 px-4"><span class="px-2 py-1 ${statusColors[app.status] || 'bg-gray-100 text-gray-600'} rounded-full text-xs">${statusText[app.status] || app.status}</span></td>
                            <td class="py-3 px-4"><a href="/staff/application-details/${app.id}" class="text-[#155386] hover:text-[#40798C]"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></a></td>
                        </tr>
                    `;
                });
                tbody.innerHTML = html;
            } else {
                tbody.innerHTML = `<tr><td colspan="5" class="py-8 text-center text-gray-500">No recent applications</td></tr>`;
            }
        } catch (error) {
    ...
    async function loadAnnouncements() {
        try {
            ...
            if (data.success && data.announcements && data.announcements.length > 0) {
                    ...
                    html += `
                        <div class="p-4 ${ann.color_class.split(' ')[0]} rounded-lg border ${ann.color_class.split(' ')[1]} hover:shadow-md transition">
                            <div class="flex items-start gap-3">
                                <div class="w-8 h-8 ${ann.color_class.split(' ')[0]} rounded-full flex items-center justify-center flex-shrink-0"><svg class="w-4 h-4 ${ann.color_class.split(' ')[2]}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${ann.icon}" /></svg></div>
                                <div class="flex-1">
                                    <div class="flex items-center justify-between"><p class="text-sm font-semibold text-gray-800">${ann.title}</p><span class="text-xs text-gray-400">${ann.time_ago}</span></div>
                ...
                container.innerHTML = `<div class="p-8 bg-gray-50 rounded-lg text-center text-gray-500">No announcements yet</div>`;
            }
        } catch (error) {
            console.error('Error loading announcements:', error);
            document.getElementById('announcements-container').innerHTML = `<div class="p-4 bg-red-50 rounded-lg text-center text-red-600">Failed to load announcements</div>`;
        }
    }
    ...
    async function createAnnouncement(event) {
        ...
        try {
            const response = await fetch('/admin/announcements', {
                ...
                body: JSON.stringify({ title, content, color, notify_users: notifyUsers })
            });
            const data = await response.json();
            if (data.success) {
                ...
                showSuccessModal('Announcement posted successfully' + (notifyUsers ? ' and notifications sent!' : '!'));
            } else {
    ...
    function setupModals() {
        ...
        if (modal) modal.addEventListener('click', function(e) { if (e.target === modal) closeAnnouncementModal(); });
...
<style>
    ...
    #weekly-bars { scrollbar-width: thin; scrollbar-color: #155386 #e5e7eb; }
    #weekly-bars::-webkit-scrollbar { height: 6px; }
    #weekly-bars::-webkit-scrollbar-track { background: #e5e7eb; border-radius: 3px; }
    #weekly-bars::-webkit-scrollbar-thumb { background: #155386; border-radius: 3px; }
</style>
@endsection
        ...
        tableBody.innerHTML = applications.map((app, index) => {
            ...
            return `
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="py-4 px-6">
                        <span class="font-mono text-sm font-medium text-[#155386]">${escapeHtml(app.application_number || 'N/A')}</span>
                    </td>
                    <td class="py-4 px-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500">${escapeHtml(app.applicant_email || '')}</p>
                            </div>
                        </div>
                    </td>
                    <td class="py-4 px-6">
                    ...
                    <td class="py-4 px-6 text-sm text-gray-600">
                        ${escapeHtml(app.archived_by_name || 'System')}
                    </td>
                    <td class="py-4 px-6">
...
function updatePagination(pagination) {
    ...
    if (paginationInfo) {
        paginationInfo.textContent = `Showing ${start} to ${end} of ${totalItems} archived applications`;
    }
...
function openRestoreModal(id = null, isMultiple = false) {
    ...
    if (isMultiple) {
        if (messageEl) messageEl.textContent = `Are you sure you want to restore ${selectedApplications.size} application(s)? They will be moved back to active applications.`;
    } else {
        if (messageEl) messageEl.textContent = 'Are you sure you want to restore this application? It will be moved back to active applications.';
    }
...
function exportArchivedApplications() {
    ...
    try {
        window.location.href = '/staff/archived-applications/export';
        console.log('Redirect initiated to:', '/staff/archived-applications/export');
    } catch (error) {
